import  {z}  from 'zod'
 ...
 export const updateDecoSchema = z.object({
    ...
    dateOfDec:z.string().datetime({message:"date cannot be left empty"}).optional(),
    reason: z.string({message:"Reason cannot be left empty"}).min(2,{message:"Reason cannot be less than 2 characters"}).optional(),
    createdById: z.string({message:"Created User cannot be left empty"}).optional(),
    decoId:z.string({message:"Decommission Id user cannot be left empty"}).optional()
 })
