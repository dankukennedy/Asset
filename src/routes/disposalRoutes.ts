...
import { allDisposalHandler, createDisposalHandler, deleteAllDisposalHandler, deleteDisposalHandler, findDisposalHandler, updateDisposalHandler } from '../controllers/disposalController';

const router = express.Router();
router.post('/disposal', (req, res, next)=>{createDisposalHandler(req, res, next).catch(next)});
router.get('/allDisposal', (req, res, next)=>{allDisposalHandler(req, res, next).catch(next)});
